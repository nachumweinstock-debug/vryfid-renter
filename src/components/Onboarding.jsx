...
const DOC_SLOTS = [
  { id: 'gov',    label: 'Government ID',        icon: '🪪' },
  { id: 'income', label: 'Proof of Income',      icon: '💼' },
  { id: 'bank',   label: 'Bank Statements',      icon: '🏦' },
  { id: 'credit', label: 'Credit Report',        icon: '📊' },
  { id: 'other',  label: 'Additional Documents', icon: '📎' },
]

const BUDGET_PRESETS = [
  { label: 'Under $1,800', min: '0',    max: '1800' },
  { label: '$1,800 – $2,400', min: '1800', max: '2400' },
  { label: '$2,400 – $3,000', min: '2400', max: '3000' },
  { label: '$3,000+',         min: '3000', max: '9999' },
]
...
/* ── OTP input — defined outside Onboarding so it's a stable component ── */
...
/* ── Toggle — defined outside Onboarding ── */
...
/* ── Chip — defined outside Onboarding ── */
...
export default function Onboarding() {
  ...
  const [data, setData] = useState({
    ...
    budgetPreset: null,   // index into BUDGET_PRESETS
    moveInDate: '',
  ...
  function goNext() {
    if (step >= total) {
      const preset = data.budgetPreset !== null ? BUDGET_PRESETS[data.budgetPreset] : { min: '1500', max: '3000' }
  ...
  /* ─────── Step renderers — called as plain functions, NOT as <Components />.
     This prevents React from treating them as separate component types and
     unmounting/remounting between renders, which caused double-click to register. ─── */
  ...
  function renderWelcome() {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 24px 48px', maxWidth: 440, margin: '0 auto', width: '100%' }}>
        ...
        <h1 style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 38, fontWeight: 900, color: '#fff', textAlign: 'center', lineHeight: 1.15, marginBottom: 14 }}>
          Find your home,<br /><em style={{ color: '#F97316' }}>stress-free.</em>
        </h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <button onClick={goNext} style={S.ctaPrimary}>Create account</button>
          <button onClick={() => { localStorage.setItem('vryfid_onboarded', '1'); navigate('/') }} style={S.ctaGhost}>
        ...
        <div style={{ paddingTop: 56 }}>
          ...
          <div style={{ marginBottom: 14, position: 'relative' }}>
            ...
            <input type={data.showPw ? 'text' : 'password'} value={data.password}
              ...
              placeholder="Min 6 characters" style={{ ...S.input, paddingRight: 52 }}
              onFocus={e => e.currentTarget.style.borderColor = '#00C9A7'}
              onBlur={e  => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'} />
          ...
          <p style={{ fontSize: 12, color: '#6B7280', marginBottom: 20, lineHeight: 1.5 }}>
            By continuing you agree to VryfID's{' '}
            <span style={{ color: '#9CA3AF', textDecoration: 'underline', cursor: 'pointer' }}>Terms of Service</span>
            {' '}and{' '}
            <span style={{ color: '#9CA3AF', textDecoration: 'underline', cursor: 'pointer' }}>Privacy Policy</span>.
          </p>

          <button onClick={goNext} disabled={!allFilled} style={{
            ...S.ctaPrimary, opacity: allFilled ? 1 : 0.4,
  ...
  function renderPhone() {
    const canSend = data.phone.replace(/\D/g, '').length >= 10
    return (
      <div style={S.inner}>
        <div style={{ paddingTop: 56 }}>
            ...
            <>
              ...
              <div style={{ display: 'flex', marginBottom: 28 }}>
                <div style={{
                  ...
                  background: '#1B2A4A',
                  border: '1.5px solid rgba(255,255,255,0.08)', borderRight: 'none',
                  borderRadius: '10px 0 0 10px', color: '#fff', fontSize: 15, flexShrink: 0,
                }}>
                  <span>🇺🇸</span>
              ...
              <button onClick={() => { upd('showOTP', true); upd('countdown', 30); upd('canResend', false) }}
                disabled={!canSend}
                style={{ ...S.ctaPrimary, opacity: canSend ? 1 : 0.4, cursor: canSend ? 'pointer' : 'not-allowed' }}>
                Send code
              </button>
            </>
          ) : (
              ...
              {data.otpVerified ? (
                <div style={{ textAlign: 'center', padding: '24px 0' }}>
                  <div style={{
                    ...
                    background: 'rgba(0,201,167,0.12)', border: '2px solid #00C9A7',
                ...
                <>
                  ...
                  <div style={{ textAlign: 'center', marginTop: 22 }}>
                    {data.canResend ? (
                      <button onClick={() => { upd('otp', Array(6).fill('')); upd('countdown', 30); upd('canResend', false) }}
                        style={{ background: 'none', border: 'none', color: '#00C9A7', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                        Resend code
                      </button>
                    ) : (
                      <span style={{ color: '#6B7280', fontSize: 13 }}>Resend in {data.countdown}s</span>
                    )}
                  </div>
                  <p style={{ color: '#6B7280', fontSize: 12, textAlign: 'center', marginTop: 12 }}>
                    Enter any 6 digits — this is a demo
                  </p>
                </>
              )}
            </>
  ...
  function renderPrefs() {
    ...
    function toggleBed(b) {
      upd('bedrooms', data.bedrooms.includes(b) ? data.bedrooms.filter(x => x !== b) : [...data.bedrooms, b])
    }
    function toggleHood(h) {
      upd('neighborhoods', data.neighborhoods.includes(h) ? data.neighborhoods.filter(x => x !== h) : [...data.neighborhoods, h])
    }

    return (
      <div style={S.inner}>
        <div style={{ paddingTop: 56, overflowY: 'auto' }}>
          <p style={{ fontSize: 12, color: '#6B7280', marginBottom: 6 }}>Step 3 of {total - 1}</p>
          <h2 style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 28, fontWeight: 700, color: '#fff', marginBottom: 32 }}>
            Rental preferences
          </h2>

          {/* Budget — simple preset chips */}
          <div style={{ marginBottom: 22 }}>
            <label style={S.label}>Monthly budget</label>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {BUDGET_PRESETS.map((b, i) => (
                <Chip key={b.label} label={b.label}
                  active={data.budgetPreset === i}
                  onToggle={() => upd('budgetPreset', data.budgetPreset === i ? null : i)} />
              ))}
            </div>
          </div>
          ...
          <div style={{
            ...
            background: '#1B2A4A', borderRadius: 12, padding: '14px 16px', marginBottom: 32,
          }}>
            <div>
          ...
          <button onClick={goNext} style={S.ctaPrimary}>Continue</button>
        </div>
      </div>
    )
  }
  ...
  function renderVibe() {
    ...
    function toggleTag(t) {
      upd('tags', data.tags.includes(t) ? data.tags.filter(x => x !== t) : [...data.tags, t])
    }
    return (
      <div style={S.inner}>
        <div style={{ paddingTop: 56 }}>
          ...
          <p style={{ color: '#9CA3AF', fontSize: 14, marginBottom: 28 }}>This is what potential roommates will see first.</p>

          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 28 }}>
            <button onClick={() => upd('photo', data.photo ? null : 'mock')} style={{
              ...
              {data.photo
                ? <span style={{ fontSize: 32, fontFamily: '"Playfair Display", serif', fontWeight: 700, color: '#fff' }}>{data.firstName?.[0]?.toUpperCase() || '?'}</span>
                : <><span style={{ fontSize: 20 }}>📷</span><span style={{ fontSize: 11, color: '#6B7280' }}>Add photo</span></>
              }
            </button>
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
              ...
              <span style={{ fontSize: 11, color: data.bio.length > MAX_BIO - 10 ? '#FBBF24' : '#6B7280' }}>{data.bio.length}/{MAX_BIO}</span>
            </div>
            <textarea value={data.bio} onChange={e => upd('bio', e.target.value.slice(0, MAX_BIO))}
              ...
              style={{ ...S.input, height: 96, resize: 'none', lineHeight: 1.55 }}
              onFocus={e => e.currentTarget.style.borderColor = '#00C9A7'}
              onBlur={e  => e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'} />
          </div>
  ...
  function renderDocs() {
    ...
    return (
      <div style={S.inner}>
        <div style={{ paddingTop: 56 }}>
          <p style={{ fontSize: 12, color: '#6B7280', marginBottom: 6 }}>Step {data.openToRoommate ? 5 : 4} of {total - 1}</p>
          <h2 style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 28, fontWeight: 700, color: '#fff', marginBottom: 8 }}>
            Upload documents
          </h2>
          <p style={{ color: '#9CA3AF', fontSize: 14, marginBottom: 28, lineHeight: 1.5 }}>
            Upload once, share with any landlord instantly. Skip for now if you'd like.
          </p>
              ...
              return (
                <div key={d.id} onClick={() => handleDocClick(d.id)} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {uploaded && <div style={{ fontSize: 12, color: '#00C9A7', marginTop: 2 }}>sample_{d.id}.pdf</div>}
                  </div>
                  <div style={{
                    width: 28, height: 28, borderRadius: '50%',
                    background: uploaded ? '#00C9A7' : 'rgba(255,255,255,0.06)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 14, color: uploaded ? '#0D0F14' : '#6B7280', flexShrink: 0,
                  }}>{uploaded ? '✓' : '+'}</div>
                </div>
              )
            })}
    ...
    return (
      <div style={{ ...S.inner, alignItems: 'center', justifyContent: 'center', paddingTop: 64 }}>
        <div style={{
          ...
          background: 'rgba(0,201,167,0.1)', border: '2px solid #00C9A7',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          marginBottom: 28,
          animation: 'scale-in-bounce 0.5s cubic-bezier(0.34,1.56,0.64,1) both',
        }}>
          <svg width="44" height="44" viewBox="0 0 44 44" fill="none">
            <path d="M11 22l8 8 14-14" stroke="#00C9A7" strokeWidth="3"
              strokeLinecap="round" strokeLinejoin="round"
              style={{ strokeDasharray: 80, animation: 'draw-check 0.5s ease-out 0.3s both' }} />
          </svg>
        </div>

        <h2 style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 30, fontWeight: 900, color: '#fff', textAlign: 'center', marginBottom: 10, animation: 'fadeUp 0.4s ease-out 0.2s both' }}>
          Welcome to VryfID,<br />{name}!
        </h2>

        <p style={{ color: '#9CA3AF', fontSize: 15, textAlign: 'center', marginBottom: 36, lineHeight: 1.5, animation: 'fadeUp 0.4s ease-out 0.35s both' }}>
          Your search starts now. Where do you want to go?
        </p>

        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24, animation: 'fadeUp 0.4s ease-out 0.5s both' }}>
          {actions.map(a => (
            <button key={a.path} onClick={() => { localStorage.setItem('vryfid_onboarded', '1'); navigate(a.path) }}
              style={{
                background: '#1B2A4A', border: '1.5px solid rgba(255,255,255,0.07)',
                borderRadius: 12, padding: '14px 16px',
            ...
            >
              <div style={{ width: 42, height: 42, borderRadius: 10, background: `${a.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0, border: `1px solid ${a.color}30` }}>{a.icon}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <button onClick={goNext} style={{ background: 'none', border: 'none', color: '#9CA3AF', fontSize: 14, cursor: 'pointer', fontFamily: 'Inter, sans-serif', textDecoration: 'underline', animation: 'fadeUp 0.4s ease-out 0.65s both' }}>
  ...
  /* ─── Dispatch — plain function calls, not <Components /> ─── */
  function renderStep() {
    switch (stepName) {
    ...
    <div style={S.wrap}>
      ...
      {step > 1 && step < total && (
        <div style={{ height: 3, background: 'rgba(255,255,255,0.06)', flexShrink: 0 }}>
          <div style={{ height: '100%', width: `${progress}%`, background: '#00C9A7', transition: 'width 0.35s ease' }} />
        </div>
      )}
      ...
      {step > 1 && step < total && (
        <button onClick={goBack} style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          cursor: 'pointer', fontSize: 18, color: '#9CA3AF', zIndex: 100,
        }}>‹</button>
      )}

      {/* Animated container — key forces CSS re-animation on step change */}
      <div key={step} className={dir === 'fwd' ? 'onboard-fwd' : 'onboard-back'}
